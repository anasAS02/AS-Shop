...
export const Navbar = () => {
  ...
  const cart = savedCart ? JSON.parse(savedCart) : { products: [], totalAmount: 0 };
    ...
    <nav className="w-full flex justify-around items-center gap-14 p-5 max-md:justify-center max-md:flex-col bg-white ">
      ...
      <div className="w-full relative max-md:order-1">
        <input
          ...
          className="p-3 w-full border-slate-200 border-2 outline-none"
        />
        <FontAwesomeIcon
          icon={faMagnifyingGlass}
          className="absolute top-2/4 -translate-x-2/4 -translate-y-2/4 right-0 w-[14px] h-[14px] text-slate-300"
        />
        <span className="max-h-[400px] overflow-y-auto flex flex-col items-start gap-3 bg-slate-200 rounded-md">
          {searchResult &&
            searchResult.length > 0 &&
            searchKey !== '' &&
            searchResult.map((product: ProductData) => (
              <span key={product._id} className="p-5 flex items-center gap-2">
                <Image
                  src={
                    product.thumbnail?.startsWith('https://i.dummyjson.com')
                      ? product.thumbnail
                      : SHOW_IMG + product.thumbnail
                  }
                  width={100}
                  height={100}
                  alt={product.title}
      ...
      <span className="flex items-start gap-10">
        ...
        <div className="flex items-center gap-3 relative">
          <Link href="/Cart">
            <FontAwesomeIcon
              icon={faCartShopping}
              className="w-[18px] h-[18px] duration-200 text-slate-600 hover:text-green-400"
            />
            {cart.products && (
              <span className="absolute -left-3 top-0 bg-red-500 text-white text-xs w-5 h-5 flex items-center justify-center rounded-full">
        ...
        <div className="flex items-center gap-3">
          <Link href="/Favourite">
            <FontAwesomeIcon
              icon={faHeart}
              className="w-[18px] h-[18px] duration-200 text-slate-600 hover:text-red-500"
            />
          </Link>
          <span className="flex flex-col items-start gap-1 text-sm">
            <p className="text-gray-500">Favourite</p>
            <p className={`${favourites && favourites.length > 0 ? 'text-red-500' : ''}`}>
