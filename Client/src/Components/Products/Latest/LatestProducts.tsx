    ...
    <div className='flex flex-col gap-5 p-24'>
        ...
        <div className='grid grid-cols-4 max-lg:grid-cols-2 max-md:grid-cols-1 items-center gap-5'>
            {products?.slice(0, 5).map((product: ProductData) => (
              <ProductCard key={product._id} _id={product._id} discountPercentage={product.discountPercentage} title={product.title} thumbnail={product.thumbnail} category={product.category} description={product.description} price={product.price} />
            ))}
        </div>
    </div>
  )
