  ...
  const handleCheckout = async () => {
    try {
      const res = await axios.post(
        CREATE_ORDER,
        { items, totalAmount: cart?.totalAmount, email: EMAIL, orderId: orderId },
        config
      );
  ...
  return (
    !isLoggedIn ? (
      <div className="h-screen">
        <h2 className="absolute left-2/4 -translate-x-2/4 -translate-y-2/4 top-2/4 max-md:top-full font-bold text-red-500 text-3xl max-md:text-base flex flex-col items-center">
          You must be logged in
          <Link href="/Auth/Login" className="text-base max-md:text-sm text-black hover:text-yellow-500 duration-200">
            Login now
          </Link>
        </h2>
      </div>
      ...
      <div className={`${cartProducts && cartProducts?.length > 0 ? 'h-full' : 'h-screen'} w-full p-10`}>
        {!cartProducts ? (
          <h2 className="absolute left-2/4 -translate-x-2/4 -translate-y-2/4 top-2/4 max-md:top-full font-bold text-red-500 text-3xl max-md:text-base flex flex-col items-center">
            Your cart is empty
            <Link href="/Categories/All-Products" className="text-base max-md:text-sm text-black hover:text-yellow-500 duration-200">
              Shop now
            </Link>
          </h2>
        ) : (
            ...
            <div className="w-fit max-md:w-full h-screen flex flex-col items-start gap-5 max-md:justify-center max-md:items-center">
              {cartProducts && cartProducts.map((product: ProductData) => (
                <span key={product._id} className="w-full flex items-center max-md:flex-col max-md:justify-center gap-5 p-2 border-2 border-gray-100 rounded-md">
                  ...
                  <FontAwesomeIcon icon={faTimes} onClick={() => handleDeleteFromCart(product)} className="cursor-pointer md:ml-auto duration-200 hover:text-red-500" />
                </span>
              ))}
            </div>
            <div className="w-fit max-md:w-full h-fit p-5 rounded-md flex flex-col gap-5 items-center">
              ...
              <button onClick={handleCheckout} className="outline-none border-none rounded-md p-4 max-md:p-2 duration-300 bg-blue-400 hover:bg-blue-300 text-white text-center">
                Checkout
              </button>
            </div>
          </div>
