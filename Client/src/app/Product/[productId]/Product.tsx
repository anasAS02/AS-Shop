...
import { GET_PRODUCT, SHOW_IMG } from '@/Utils/Apis';
            ...
            <span className='flex max-md:flex-col-reverse items-center gap-5'>
                <span className='flex md:flex-col items-start gap-2 max-md:flex-wrap'>
                    {imgs.map((img: string, index: number) => (
                        <Image onClick={() => setImgIndes(index)} key={index} src={img.startsWith('https://i.imgur.com/') ? img : SHOW_IMG + img} width={130} height={170} alt={product.title} className={`${index === imgIndex ? 'opacity-100' : 'opacity-60'} rounded-md cursor-pointer duration-200 hover:opacity-100`} />
                    ))}
                </span>
                <Image src={imgs[imgIndex].startsWith('https://i.imgur.com/') ? imgs[imgIndex] : SHOW_IMG + imgs[imgIndex]} width={600} height={600} alt={product.title} className='rounded-md' />
            </span>
